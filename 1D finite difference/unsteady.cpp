#include <vector>
#include <list>
#include <map>
#include <set>
#include <deque>
#include <stack>
#include <bitset>
#include <algorithm>
#include <functional>
#include <numeric>
#include <utility>
#include <sstream>
#include <iostream>
#include <iomanip>
#include <cstdio>
#include <cmath>
#include <cstdlib>
#include <ctime>
#include <fstream>
#include <string.h>
using namespace std;
typedef struct temparature_data {
	int boundary_condition;
	double x,thermal_conductivity,temparature;
}temp_data;
int n,conv_stat;
temp_data *temp = new temp_data[101];
double alpha,delta_t,delta_x,A,B,a[101],b[101],d[101],source[101],var[101],error[101];
void data_initialization();
void creat_grid();
void check_convergence();
void tdma();
int main() {
	ofstream myfile2;
	myfile2.open ("result.txt");
	int i,j,k,done = 0;
	data_initialization();
	creat_grid();
	for(int ii=0;(ii<100000)&&(done != 1);ii++) {
		for(int jj=1;jj<(n-1);jj++) {
			source[jj] = -temp[jj].temparature - (A*(temp[jj+1].temparature - (2*temp[jj].temparature) + temp[jj-1].temparature));
		}
		tdma();
		for(int jj = 1;jj<n;jj++) {
			error[jj] = abs(var[jj] - temp[jj].temparature);
			temp[jj].temparature = var[jj];
		}
		if(ii%100 == 0) {
			for(int kk=0;kk<n;kk++) {
				myfile2 << temp[kk].temparature<<endl;
			}
		}
		conv_stat = 0;
		check_convergence();
		if(conv_stat == 0) {
			cout << "Solution converged at timestep"<<" "<<ii+1<<endl;
			for(int kk=0;kk<n;kk++) {
				myfile2 << temp[kk].temparature<<endl;
			}
			done = 1;
		}
		//cout << "Time Step  "<< ii<<"  "<<"computation done !!"<<endl;
	}
	myfile2.close();
	if(done == 1) {
		cout << "Solution converged !!!\n";
	}else
		cout << "Solution Not Converged More iterations required !!\n";

	//delete[] temp;

}
void creat_grid() {
	ofstream myfile1;
	myfile1.open ("grid.txt");
	//myfile1 << "Writing this to a file.\n";
	for(int i = 1;i<n;i++) {
		temp[i].x = delta_x*(i);
		temp[i].boundary_condition = 0;
	}
	temp[0].boundary_condition = 1; temp[n-1].boundary_condition = 1;	
	for(int i=0;i<n;i++) {
		myfile1 << temp[i].x<<"  "<<temp[i].boundary_condition<<"  "<<temp[i].temparature<<endl;
	}
	myfile1.close();
}
void data_initialization() {
	int i,j;
	n = 101;
	delta_x = 0.01; alpha = 0.000111; delta_t = 0.10;
	A = (alpha*delta_t)/(2*delta_x*delta_x);
	B = 1 + ((alpha*delta_t)/(delta_x*delta_x));
	for(i=1;i<(n-1);i++) {
		d[i] = -B;
	}
	d[0] = 1; d[n-1] = 1;
	for(i=1;i<(n-1);i++) {
		a[i] = -A;
	}
	a[0] = 0.0; a[n-1] = 0.0;
	for(i=1;i<(n-1);i++) {
		b[i] = -A;
	}
	b[0] = 0.0; b[n-1] = 0.0;
	source[0] = 100; source[n-1] = 0;
	for(i=1;i<(n-1);i++) {
		temp[i].temparature = 0;
	}
	temp[0].temparature = 100; temp[n-1].temparature = 0;
}
void tdma() {
	int i,j;
	double aa[n],cc[n];
	aa[0] = 0; aa[n-1] = 0;
	for(i=1;i<(n-1);i++) {
		aa[i] = a[i]/(d[i] - (b[i]*aa[i-1]));
	}
	cc[0] = source[0]; cc[n-1] = source[n-1];
	for(i=1;i<(n-1);i++) {
		cc[i] = (b[i]*cc[i-1] + source[i])/(d[i] - (b[i]*aa[i-1]));
	}
	var[n-1] = source[n-1];
	for(i=n-1;i>0;i--) {
		var[i] = (aa[i]*var[i+1]) + cc[i];
	}
}
void check_convergence() {
	for(int i=0;i<n;i++) {
		if(error[i] >= 0.00001) {
			conv_stat += 1;
		}
	}
}
//void write_data() {
	//myfile2 << "Writing this to a file.\n";
//	for(int i=0;i<n;i++) {
//		myfile2 << temp[i].temparature<<endl;
//	}
//	myfile2.close();
//}
